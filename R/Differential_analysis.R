...
TSNEPlot.1(object = SSCs_Spermato, no.legend = F, do.label = F, pt.size = 1,
...
# FeatureHeatmap==========
FeatureHeatmap_save <- function(object,subset_dev_mark,ident.use, Top_n= 100){
        subset_dev_mark_top100 <- subset_dev_mark %>% group_by(cluster) %>% 
                top_n(Top_n, avg_logFC) 
        subset_dev_mark_top100 <- as.character(subset_dev_mark_top100$gene)
        for (gene in subset_dev_mark_top100) {
                temp_plot = Customize_Seurat_FeatureHeatmap(object, gene)+
                        theme(text = element_text(size=15))
                ggsave(temp_plot, file=paste0("./output/20180812/tsne_matrix/",
                                              ident.use,"/",
                                              gene,".jpeg"),
...
FeatureHeatmap_save(object = Spermato, subset_dev_mark = Spermatogonia_dev_mark,
                    ident.use = "Spermatogonia")
...
FeatureHeatmap_save(object = Spermato, subset_dev_mark = Early_spermatocytes_dev_mark,
                    ident.use = "Early spermatocytes")
...
FeatureHeatmap_save(object = Spermato, subset_dev_mark = Spermatocytes_dev_mark,
                    ident.use = "Spermatocytes")
