...
DoHeatmap.1 <- function(object, marker_df, add.genes = NULL, Top_n = 10, group.order = NULL,ident.use,
    ...
    if(!is.null(add.genes)) {
        top_gene = c(add.genes, as.character(top$gene))
...
SingleFeaturePlot.1 <- function (object = object, feature = feature, pt.size = 1.0,
                                 ...
                                 dark.theme = FALSE,title="", do.return = FALSE,...) 
{
    dim.code <- GetDimReduction(object = object, reduction.type = reduction.use, 
                                slot = "key")
    dim.codes <- paste0(dim.code, c(dim.1, dim.2))
...
SplitTSNEPlot <- function(object, split.by = "conditions",
                          select.plots = NULL, do.return = TRUE, colors.use = NULL,
    ...
    for(i in 1:length(select.plots)){
        p[[i]] <- TSNEPlot.1(object = object.subsets[[select.plots[i]]],
                             #colors.use = ExtractMetaColor(object.subsets[[select.plots[i]]]),
    ...
    if(do.return) return(p) else print(do.call(cowplot::plot_grid, p))
}
...
#' @param ... all other parameters are inherited from Seurat::TSNEPlot()
#' @export p ggplot object from TSNEplot
#' @example SplitDimPlot(mouse_eyes, split.by = "conditions")
#' @example SplitDimPlot(mouse_eyes, split.by = "orig.ident", select.plots = c(2,1))
SplitDimPlot <- function(object = object, split.by = "conditions",
                         reduction.use= "TSNE",
    ...
    if(is.null(select.plots)) select.plots <- 1:length(levels)
    for(i in 1:length(select.plots)){
        p[[i]] <- DimPlot.1(object = object.subsets[[select.plots[i]]],
